...
from .common import (
    PackageType, Version, EnvironmentInformation, ReleaseFile, PytestRunner
)
from .git import HomebrewRepository
from .virtualenvs import EnvBuilder, DBTPackageEnv, PipInstalledDbtEnv
...
class HomebrewTemplate:
    ...
    def store_artifacts(
        self, env: EnvironmentInformation
    ) -> None:
...
def _tgz_to_name(path: Path) -> str:
    with tarfile.open(path) as archive:
        names = [
            n for n in archive.getnames() if n.split('/')[-1] == 'PKG-INFO'
        ]
...
class HomebrewLocalBuilder(BaseHomebrewBuilder):
    ...
    def make_venv(self, path: Path) -> EnvBuilder:
        # homebrew can't gracefully handle wheels
        env = DBTPackageEnv(
            package_dir=self.package_dir, ext=PackageType.Sdist
        )
        env.create(path)
    ...
    def get_packages(self) -> Iterator[HomebrewDependency]:
        dbt_tgzs = {
            _tgz_to_name(w): w
            for w in self.package_dir.glob('*.tar.gz')
        }
    ...
    def run_tests(self, formula_path: Path, audit: bool = True):
        ...
        stream_output([
            python_bin, '-m', 'pip', 'install', '-r', dev_requirements
        ])
    ...
    @classmethod
    def from_env_info(
        cls, env: EnvironmentInformation
...
def add_homebrew_parsers(subparsers):
    homebrew_sub = subparsers.add_parser(
        'homebrew',
        help='Homebrew operations'
    )
    homebrew_subs = homebrew_sub.add_subparsers(
        title='Available sub-commands'
    )

    homebrew_test_sub = homebrew_subs.add_parser(
        'test',
        help='Test the homebrew package',
    )
    homebrew_test_sub.set_defaults(func=homebrew_test)

    homebrew_upload_sub = homebrew_subs.add_parser(
        'upload',
        help='Upload the homebrew package',
    )
