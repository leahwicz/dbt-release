...
from .virtualenvs import (
    EnvBuilder, DevelopmentWheelEnv, DBTPackageEnv, PackagingEnv
)
...
def make_requirements_txt(
    env_dir: Path, dbt_dir: Path, requirements_path: Path
):
    ...
    reqenv = EnvBuilder(with_pip=True)
...
def create_build_commit(args=None):
    ...
    make_requirements_txt(
        env.linux_requirements_venv, env.dbt_dir, requirements_path
    )
...
def add_native_parsers(subparsers):
    native_sub = subparsers.add_parser(
        'native',
        help='build the wheels/tarfiles'
    )
    ...
    pkg_sub = native_subs.add_parser(
        'package',
        help='build the wheels/tarfiles'
    )
    pkg_sub.set_defaults(func=build_wheels)

    test_sub = native_subs.add_parser(
        'test',
        help='Run the given tests'
    )
    ...
    upload_sub = native_subs.add_parser(
        'upload',
        help='Upload the package to pypi'
    )
